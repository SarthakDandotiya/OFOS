...
menuList.preloadData = () => {
	...
	// Fetch the menu data.
	httpClient
		.request({ path: '/api/menus', method: 'GET' })
        ...
        `;
				...
				document
					...
					.addEventListener('click', event => {
						...
						const existingItem = basketItems
							.filter(basketItem => basketItem.id === menuItem.id)
							.pop();
						...
						shoppingCart
							.updateCart(httpClient, menuItem.id, newQuantity)
							.then(shoppingCart => {

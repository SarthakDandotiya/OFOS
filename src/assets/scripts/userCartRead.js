...
userCartRead.shoppingCartUpdate = async (itemId, quantity) => {
	return new Promise((resolve, reject) => {
		shoppingCart
			.updateCart(httpClient, itemId, quantity)
			.then(shoppingCart => {
...
userCartRead.preloadData = async () => {
	// Load menu items.
	const { statusCode, responsePayload: menuItems } = await httpClient.request(
	...
	if (shoppingCart.cart.items && shoppingCart.cart.items.length) {
		...
		shoppingCart.cart.items.forEach(cartItem => {
			const cartItemObject = menuItems
				.filter(menuItem => menuItem.id === cartItem.id)
				.pop();
      ...
      `;
			...
			document
				...
				.addEventListener('click', event => {
					userCartRead
						.shoppingCartUpdate(cartItem.id, 0)
						.then(shoppingCart => {
			...
			document
				...
				.addEventListener('change', event => {
					...
					userCartRead
						.shoppingCartUpdate(cartItem.id, newQuantity)
						.then(shoppingCart => {
							// Preload data for the current page.
							userCartRead.preloadData().then(() => {});
							app.drawShoppingCartCounter(shoppingCart);
